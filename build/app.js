...
class GameCoordinator {
  ...
  gameOver() {
    localStorage.setItem('highScore', this.highScore);
    this.displayrealText(
      {
        left: this.scaledTileSize * 9.5,
    ...
    new Timer(() => {
      ...
      window.location.replace("https://www.firefighter.com.my/blogs/news/ultimate-fire-safety-tips");
    }, 5000);
  }
  ...
  displayrealText(position, duration, width, height) {
    const pointsDiv = document.createElement('div');

    pointsDiv.style.position = 'absolute';
    pointsDiv.style.backgroundSize = `${width}px`;
    pointsDiv.textContent ="PLAY SAFE WITH FIRE";
    pointsDiv.style.width = `${width}px`;
    pointsDiv.style.height = `${height || width}px`;
    pointsDiv.style.top = `${position.top}px`;
    pointsDiv.style.left = `${position.left}px`;
